      ...
      setImprovements(
        data.ai_suggestions.map((s: any) => {
          ...
          if (s.area.includes("Response Time")) {
            currentDisplay = `${s.current.toFixed(2)} min`;
            targetDisplay = `${s.target.toFixed(2)} min`;
          } else if (s.area.includes("Resolution") || s.area.includes("Satisfaction")) {
            currentDisplay = `${Math.round(s.current)}%`;
            targetDisplay = `${Math.round(s.target)}%`;
          } else {
                ...
                <div className="py-6">
                  <div className="mb-8">
                    <div className="flex items-center justify-between mb-4">
                      <h3 className="text-lg font-semibold text-ai-text flex items-center">
                        <Check className="w-5 h-5 text-ai-green mr-2" />
                        {uploadedFiles.length} file{uploadedFiles.length > 1 ? 's' : ''} ready for analysis
                      </h3>
                      <Button
                        variant="outline"
                        onClick={handleUploadLogs}
                        className="border-ai-blue text-ai-blue hover:bg-ai-blue/10 px-4 py-2 rounded-xl shadow-sm flex items-center"
                      >
                    ...
                    <div className="space-y-3 max-h-64 overflow-y-auto pr-2">
                      {uploadedFiles.map((file, index) => (
                        <div key={index} className="flex items-center justify-between p-3 bg-ai-bg/50 rounded-lg border border-ai-border/50 hover:shadow-sm transition-shadow">
                          <div className="flex items-center space-x-3 truncate">
                            <FileText className="w-5 h-5 text-ai-blue flex-shrink-0" />
                            <span className="text-ai-text truncate">{file.name}</span>
                            <span className="text-sm text-ai-text-light flex-shrink-0">{(file.size / 1024).toFixed(1)} KB</span>
                          </div>
                          <Button
                            variant="ghost"
                            size="sm"
                            className="text-ai-text-light hover:text-ai-red hover:bg-ai-red/10 flex-shrink-0"
                  ...
                  <div className="flex justify-center space-x-4">
                    <Button
                      ...
                      className="border-ai-blue text-ai-blue hover:bg-ai-blue/10 px-6 py-3 rounded-xl shadow-sm"
                    >
                      Clear All Files
                    </Button>
                    <Button
                      ...
                      className="bg-gradient-to-r from-ai-blue to-ai-green hover:from-ai-blue/90 hover:to-ai-green/90 text-white px-8 py-3 rounded-xl shadow-elegant hover:shadow-glow transition-all duration-300 transform hover:scale-105 disabled:opacity-70 disabled:transform-none"
                    >
                      {processing ? (
                        <>
              ...
              <CardContent>
                <div className="space-y-6">
                  {improvements.map((item, index) => {
                    const progressValue = (item.current / item.target) * 100;
                    return (
                      <div
                        key={index}
                      ...
                      >
                        ...
                        <div className="relative h-3 w-full overflow-hidden rounded-full bg-ai-blue-light/30 mb-4">
                          <div
